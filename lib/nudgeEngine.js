...
function pickFallbackNudge(context, lastMessage = '') {
  if (context?.activeTopic) {
    return clampText(`¿Seguimos con lo de ${context.activeTopic}?`, 120);
  }

  const pool = GENERIC_LINES.filter((line) => line !== lastMessage);
...
async function generateAutoNudge({ apiKey, context, session, summary = '', lastMessage = '' } = {}) {
  ...
  if (!apiKey) {
    const fallbackMessage = pickFallbackNudge(context, lastMessage);
  ...
  const messages = [
    ...
    ...((context?.modelMessages || []).slice(-10)),
    {
      role: 'system',
  ...
  try {
    ...
    console.info('[yumiko][auto-nudge] generation result', { mode: 'llm', message: content });
    return content;
  } catch (error) {
    const fallbackMessage = pickFallbackNudge(context, lastMessage);
